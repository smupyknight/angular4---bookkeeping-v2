...
class IncomeComparisonController extends BaseController
{
    public function index( $period, $account ) {
    	...
    	foreach ( $date_segments as $date_segment ) {
    		$this_year = Sales::whereBetween( 'date', [ $date_segment[ 'this_year' ][ 'from' ], $date_segment[ 'this_year' ][ 'to' ] ] );
    		$last_year = Sales::whereBetween( 'date', [ $date_segment[ 'last_year' ][ 'from' ], $date_segment[ 'last_year' ][ 'to' ] ] );
    	...
    	return RestResponseMessages::reportRetrieveSuccessMessage( 'Income Comparison', $content );
    }
}
